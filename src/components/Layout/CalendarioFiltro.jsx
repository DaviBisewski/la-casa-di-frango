  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`
          flex items-center gap-3 sm:gap-6 border-2 rounded-2xl
          px-5 sm:px-15 py-3 sm:py-6
          text-lg sm:text-3xl font-semibold transition-all duration-200
          ${dataSelecionada
      ...
      >
        <img
          src={calendarioIcon}
          alt="Calendário"
          className={`w-5 h-5 sm:w-10 sm:h-10 transition-all ${
            dataSelecionada ? "brightness-0 invert" : ""
          }`}
        />
      ...
      {aberto && (
        ...
        >
          ...
          <div
            className="relative bg-white z-10 w-full h-full flex flex-col overflow-y-auto"
            style={{
          ...
          >
            ...
            {/* Header */}
            <div className="flex items-center justify-between px-5 sm:px-12 py-6 sm:py-10 border-b border-[#0F4C3A]/10">
              <h2 className="text-[#0F4C3A] text-2xl sm:text-5xl font-extrabold">
                Filtrar por data
              </h2>

              <button
                onClick={() => setAberto(false)}
                className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-[#0F4C3A]/10 flex items-center justify-center hover:bg-[#0F4C3A]/20 transition-all"
              >
                {/* X icon */}
                <svg
                  viewBox="0 0 24 24"
            ...
            <div className="px-5 sm:px-12 pt-6 sm:pt-10 pb-4 sm:pb-6">
              <label className="text-[#0F4C3A] text-base sm:text-2xl font-semibold mb-3 block">
                Digite a data:
              </label>
              ...
              <div className="relative">
                ...
                {inputErro && (
                  <p className="text-red-500 text-sm sm:text-xl mt-2 ml-2">
                    Data inválida ou sem expediente registrado
                  </p>
                )}

                <p className="text-[#0F4C3A]/40 text-sm sm:text-xl mt-2 ml-2">
            ...
            {/* Navegação mês/ano */}
            <div className="flex items-center justify-between px-5 sm:px-12 py-5 sm:py-8 gap-3">
              <button
                onClick={mesAnterior}
                className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-[#D4F1E6] flex items-center justify-center hover:bg-[#0F4C3A]/20 active:scale-95 transition-all flex-shrink-0"
              >
                {/* Seta esquerda */}
                <svg
                  viewBox="0 0 24 24"
              ...
              <span
                className="text-[#0F4C3A] text-lg sm:text-4xl font-extrabold text-center"
                key={`${mes}-${ano}`}
            ...
            {/* Dias da semana */}
            <div className="grid grid-cols-7 px-3 sm:px-12 mb-2">
              {DIAS_SEMANA.map((d) => (
                <span
                  key={d}
                  className="text-center text-[#0F4C3A]/40 text-xs sm:text-xl font-bold py-2 sm:py-3"
                >
                  {d}
                </span>
              ))}
            ...
            {/* Grade de dias */}
            <div
              className="grid grid-cols-7 gap-1 sm:gap-3 px-3 sm:px-12"
              key={`grade-${mes}-${ano}`}
              ...
              {dias.map((dia, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={!temExp}
                    className={`
                      relative aspect-square rounded-xl sm:rounded-2xl flex flex-col items-center
                      justify-center transition-all duration-200 text-sm sm:text-2xl font-bold
                      ${selecionado
                      ...
                      ${ehHoje && !selecionado ? "ring-2 sm:ring-3 ring-[#0F4C3A]/40" : ""}
                    `}
                  >
                    {dia}
                    ...
                    {/* Ponto indicador */}
                    {temExp && !selecionado && (
                      <span className="absolute bottom-1 sm:bottom-2 w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[#0F4C3A]/50" />
                    )}
                  </button>
                );
              })}
            ...
            <div className="px-5 sm:px-12 py-6 sm:py-12 mt-auto">
              {dataSelecionada && (
                <button
                  onClick={handleLimpar}
                  className="w-full py-5 sm:py-8 rounded-2xl border-2 border-[#0F4C3A]/20
                             text-[#0F4C3A] text-lg sm:text-3xl font-semibold mb-4 sm:mb-6
              ...
              <button
                onClick={() => setAberto(false)}
                className="w-full py-5 sm:py-8 rounded-2xl bg-[#0F4C3A] text-white
                           text-lg sm:text-3xl font-bold hover:bg-[#0a3528]
